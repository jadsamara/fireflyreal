...
export const CancelledSparkCard = ({ spark = {}, navigation, userNumber }) => {
  const {
  ...
  useEffect(() => {
    ...
    const getPenalty = async () => {
      const userInfoForPenalty = await getUserInfo(userNumber);
      const sparkDocID = spark.documentId;
      const penaltyObject = userInfoForPenalty.penalties.find(
        (p) => p.sparkDocID === sparkDocID
      );
      const penalty = penaltyObject ? penaltyObject.penalty : null;
      setPenalty(penalty);
    };
  ...
  const onHandleNavigateToCancelledSparkPage = () => {
    navigation.navigate("CancelledSparkPage", {
      spark,
      penalty,
    });
  };
  ...
  return (
    <Container>
      <SparkImageContainer onPress={onHandleNavigateToCancelledSparkPage}>
        <SparkImage
          source={{
            uri: sparkImage,
          }}
