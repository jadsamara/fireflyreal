...
export const NoshowSparkCard = ({ spark = {}, navigation }) => {
  const {
  ...
  const onHandleNavigateToCancelledSparkPage = () => {
    navigation.navigate("NoshowSparkPage", {
      spark,
    });
  };
  ...
  const renderRefundText = () => {
    return (
      <MoneyDepositContainer>
        <MoneyDepositText>Whole deposit lost (-{luminsPrice})</MoneyDepositText>
  ...
  return (
    <Container>
      ...
      <SparkStatusContainer>
        <SparkStatusText>No Show</SparkStatusText>
      </SparkStatusContainer>
      <LineBreak />
    </Container>
  );
